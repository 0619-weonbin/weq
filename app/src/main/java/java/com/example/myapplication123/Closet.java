...
import com.example.myapplication123.CloTag;
import com.example.myapplication123.MyDatabaseHelper;
import com.example.myapplication123.R;
import com.example.myapplication123.Style;
...
public class Closet extends AppCompatActivity {
    ...
    private void showClo(String TmpTag,String StyTag) {
        ...
        for (CloTag a : filter){
            ...
            Log.d("ClosetImagePath", "CloUrl: " + a.CloUrl);

            File file = new File(a.CloUrl);
            Log.d("ClosetImageExists", "file.exists(): " + file.exists());

            Glide.with(Closet.this)  // 또는 getApplicationContext()
                    .load(new File(a.CloUrl)) // 또는 new File(a.CloUrl)
                    .override(300, 300) // 크기 지정
                    .into(imageView);
